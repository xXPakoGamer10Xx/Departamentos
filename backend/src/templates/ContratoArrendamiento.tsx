...
export function ContratoArrendamiento({ data }: ContratoProps) {
    ...
    ? (() => {
        ...
        if (deposito_tipo === 'quincenas' || deposito_tipo === 'personalizado') {
          const fechasStr = Array.isArray(deposito_fechas) && deposito_fechas.length > 0
            ...
            : '';
          return `El arrendatario entregará al arrendador, por concepto de compensación, la cantidad de ${depositoAmount}, pagadera en ${metodoPagoLabel}, en abonos diferidos con vencimiento en las siguientes fechas: ${fechasStr}. Esta compensación es adicional a la renta mensual y, conforme a lo pactado en la cláusula de Compensación no reembolsable, en ningún caso será devuelta ni aplicada como pago de rentas.`;
        }
        return `El arrendatario entregará al arrendador, por concepto de compensación, la cantidad de ${depositoAmount}, pagadera en ${metodoPagoLabel} al inicio del presente contrato. Esta compensación es adicional a la renta mensual y, conforme a lo pactado en la cláusula de Compensación no reembolsable, en ningún caso será devuelta ni aplicada como pago de rentas.`;
      })()
    : null;
  ...
  const clauses = [
    ...
    `Compensación no reembolsable. La cantidad entregada por concepto de compensación no será devuelta al arrendatario bajo ninguna circunstancia, sea cual sea la causa de terminación del contrato (vencimiento natural, desocupación anticipada o incumplimiento). Dicha cantidad es adicional e independiente de la renta mensual, no tiene el carácter de depósito en garantía ni de fondo para mantenimiento, pintura o restauración del inmueble, y tampoco podrá aplicarse como pago de mensualidades de renta. Cualquier daño, deterioro o reparación imputable al arrendatario será cubierto por éste de manera adicional y por separado, conforme a lo dispuesto en la cláusula sexta de este contrato.`,
